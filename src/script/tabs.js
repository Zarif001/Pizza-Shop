import axios from 'axios'
    ...
    const renderItems = (arrItems) => {
        ...
        tabsCart.forEach((item, i) => {
            item.addEventListener('click', (e) => {
                ...
                let img = parent.querySelector('.main__tabs-img').src
                ...
                axios.post('https://62d14da4dccad0cf1764cae4.mockapi.io/Pizza', array)
    ...
    const getData = async (category) => {
        await axios.get('https://pizza-3ba7e-default-rtdb.firebaseio.com/db.json')
        .then(function (res) {
            const arr = category ? res.data.filter((item) => item.category === category) : res.data
